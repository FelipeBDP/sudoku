...
import br.com.dio.model.SpaceConfig;
    ...
    private List<List<Space>> initBoard(final Map<String, String> gameConfig) {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                var config = SpaceConfig.parse(gameConfig.get("%s,%s".formatted(i, j)));
                spaces.get(i).add(new Space(config.expected(), config.fixed()));
            }
        }
